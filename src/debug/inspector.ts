...
export interface DebugInspector {
  ...
  // Execution control
  step(): boolean; // Returns true if breakpoint hit
  run(): void;
  stop(): void;
  reset(): void;
...
export class DebugInspectorImpl implements DebugInspector {
  private breakpoints = new Set<number>();
  private tracingEnabled = false;
  private trace: TraceEntry[] = [];
  private maxTraceEntries = 1000;
  ...
  private recordTraceEntry(address: number, opcode: number, registers: CPUState): void {
  ...
  private disassembleInstruction(address: number, opcode: number): string {
